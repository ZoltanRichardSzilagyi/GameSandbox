...
THREE.MD2CharacterComplex = function() {
	...
	this.updateMovementModel = function(delta) {
		...
		// displacement
		// collision detection added, not perfect
		if (this.speed > 1 || this.speed < 0) {
			...
			var sinOrientation = Math.sin(this.bodyOrientation);
			var cosOrientation = Math.cos(this.bodyOrientation);

			var newX = x + sinOrientation * forwardDelta;
			var newZ = z + cosOrientation * forwardDelta;
			var farX = x + sinOrientation * 900;
			var farZ = z + cosOrientation * 900;
			var move = true;
			
			if (x != newX || newZ != z) {
				var originPoint = new THREE.Vector3(x, y, z);
				var directionVector = new THREE.Vector3(farX, y,farZ);
